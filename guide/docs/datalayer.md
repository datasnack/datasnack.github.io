...
To create a new Data Layer you need first to decide on a unique identifier for it -- the `key`. A common convention is to use something like `<source>_<paramter>`, i.e. `worldpop_popd` (a source from **WorldPop** describing **pop**ulation **d**ensity).
...
Int the following tables the distinction between the source based and database based information is shown. As well as the option to document the orignal data source.
